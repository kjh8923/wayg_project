...
function displayPlaces(places) {
   ...
   fragment = document.createDocumentFragment(), //새로운 빈 DocumentFragment 를 생성. DocumentFragment 인터페이스는 부모가 없는 아주 작은 document 객체를 나타냄. 
   bounds = new kakao.maps.LatLngBounds(), //LatLngBounds - WGS84 좌표계에서 사각영역 정보를 표현하는 객체를 생성
   ...
   for ( var i=0; i<places.length; i++ ) {
       ...
       // 마커를 생성하고 지도에 표시
       var placePosition = new kakao.maps.LatLng(places[i].y, places[i].x), //LatLng - WGS84 좌표 정보를 가지고 있는 객체를 생성
       ...
       (function(marker, title, address, category, category_code) {
          //addListener(target, type, handler) - 다음 지도 API 객체에 이벤트를 등록
          ...
          kakao.maps.event.addListener(marker, 'mouseover', function() { //마커에 마우스 올렸을 때
           ...
           itemEl.onmouseout =  function () { // 검색목록에 마우스 치웠을 때 infowindow 닫기
               infowindow.close();
           };
   ...
   map.setBounds(bounds); //setBounds() - 주어진 영역이 화면 안에 전부 나타날 수 있도록 지도의 중심 좌표와 확대 수준을 설정
}
...
function inputdata(marker, target1, value, title, address, category, category_code) {
   ...
   var imageSrc = '/init/images/marker.png', // 마커이미지의 경로    
    ...
    imageOption = {offset: new kakao.maps.Point(10, 69)}; // 마커이미지의 옵션. 마커의 좌표와 일치시킬 이미지 안에서의 좌표를 설정
   
   var markerImage = new kakao.maps.MarkerImage(imageSrc, imageSize, imageOption), //마커 이미지 옵션을 markerImage객체에 담기
    markerPosition = new kakao.maps.LatLng(marker.getPosition().getLat(), marker.getPosition().getLng()); // 마커가 표시될 위치
